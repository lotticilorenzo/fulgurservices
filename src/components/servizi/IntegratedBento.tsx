...
function BentoCard({ index, title, img, icon: Icon, overlayOpacity = "bg-black/40" }: { index: number, title: string, img: string, icon: any, overlayOpacity?: string }) {
  return (
    <ScrollReveal delay={index * 0.1}>
      <SpotlightCard className="relative min-h-[200px] rounded-2xl overflow-hidden group">
        <Image 
          src={img} 
          alt={title}
          fill
        ...
        {/* Minimal Icon */}
        <div className="absolute top-4 right-4 z-20 text-white/50">
          <Icon size={20} weight="regular" />
        </div>

        {/* Title */}
        <div className="absolute bottom-6 left-6 z-20">
          <h4 className="font-display text-lg font-bold text-white transition-colors group-hover:text-[var(--accent)]">
            {title}
          </h4>
        </div>
      </SpotlightCard>
