...
export function TrustBar() {
  return (
    <section className="bg-white border-y border-[var(--br)]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4">
          {TRUST_DATA.map((item, i) => (
            <ScrollReveal key={i} delay={i * 0.1}>
              <Link 
                href={item.link}
                className={`p-8 md:p-12 flex flex-col items-start border-t-[3px] border-transparent hover:border-[var(--accent)] transition-all duration-500 group relative block h-full ${i < TRUST_DATA.length - 1 ? 'md:border-r border-[var(--br)]' : ''}`}
              >
                <div className="absolute top-0 left-0 w-full h-[3px] bg-[var(--accent)] opacity-100" />
                
                <div className="font-display text-[clamp(40px,5vw,56px)] font-extrabold text-[var(--accent)] mb-3 tracking-tight">
                  {item.isText ? (
                    item.value
                  ) : (
                    <CounterUp 
